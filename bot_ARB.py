...
symbol = 'ARBUSDT'  # Cặp giao dịch
volume = 20        # Khối lượng mỗi giao dịch
upper = 65          # RSI upper để mở Short
lower = 26          # RSI lower để mở Long
    ...
    while True:
        ...
        if position is None:
            if current_rsi < lower:
                # Xác nhận tín hiệu tăng: Giá hiện tại phải cao hơn giá trước đó
                ...
                # Xác nhận tín hiệu giảm: Giá hiện tại phải thấp hơn giá trước đó
                if current_price < previous_price:
                    position = 'short'
                    entry_price = current_price
                    stop_loss_price = round(entry_price + (1.5 * atr), 4)
